...
The `children` prop is already rendered by the time it reaches the Client Component. The Client Component doesn't import or execute the Server Component—it just receives its output.

```
Server renders:
  Page
    └─ ClientWrapper
         └─ ServerContent (rendered here!)
...
app/
└── products/
    └── page.tsx                    (Server - fetches products)
        ├── ProductList.tsx         (Server - renders list)
        │   └── ProductCard.tsx     (Server - renders card)
        │       └── AddToCart.tsx   (Client - button interaction)
        └── FilterPanel.tsx         (Client - filter state)
```
...
```tsx
// ❌ Whole component becomes client
"use client";
...
// ❌ Date objects can't be serialized
