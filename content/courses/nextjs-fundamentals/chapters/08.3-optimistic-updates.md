...
Optimistic approach:
```
User clicks → Update UI immediately → Wait for server
...
export function LikeButton({ postId, isLiked, likeCount }) {
  ...
  return (
    <button onClick={handleClick}>
      {optimisticLike.isLiked ? '❤️' : '🤍'} {optimisticLike.likeCount}
    </button>
  );
}
```
...
```tsx
// ✅ Good: Visual feedback for pending items
{optimisticItems.map(item => (
  <li 
    key={item.id}
...
```tsx
// ✅ Good: Temporary ID for new items
addOptimisticItem({
